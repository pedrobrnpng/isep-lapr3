...
public class GraphAlgorithmsTest {
    ...
    private void printLL(LinkedList<String> path) {
        String read="";
        Iterator<String> i=path.iterator();
        while (i.hasNext()) {
            read+=i.next().toString()+" ";
        }
    ...
    public void testAllPaths() {
        ...
        paths=GraphAlgorithms.allPaths(completeMap, "Porto", "Porto");
        assertTrue(paths.size()==3);
        
        paths=GraphAlgorithms.allPaths(incompleteMap, "Porto", "Porto");
        assertTrue(paths.size()==3);
        
        paths=GraphAlgorithms.allPaths(incompleteMap, "Porto", "LX");
        assertTrue(paths.isEmpty());
        
        paths=GraphAlgorithms.allPaths(incompleteMap, "Porto", "Castelo Branco");
        assertTrue(paths.isEmpty());
    ...
    */
	...
	double lenpath=0;
        lenpath=GraphAlgorithms.shortestPath(completeMap,"Porto","LX",shortPath,String.class);
        assertTrue("Length path should be 0 if vertex does not exist", lenpath == 0);
	
        lenpath=GraphAlgorithms.shortestPath(incompleteMap,"Porto","Faro",shortPath,String.class);
	assertTrue("Length path should be 0 if there is no path", lenpath == 0);
		
        lenpath=GraphAlgorithms.shortestPath(completeMap,"Porto","Porto",shortPath,String.class);
        assertTrue("Number of nodes should be 1 if source and vertex are the same", shortPath.size() == 1);
		
	lenpath=GraphAlgorithms.shortestPath(incompleteMap,"Porto","Lisboa",shortPath,String.class);
	...
	lenpath=GraphAlgorithms.shortestPath(incompleteMap,"Braga","Leiria",shortPath, String.class);
        ...
        shortPath.clear();
        lenpath=GraphAlgorithms.shortestPath(completeMap,"Porto","Castelo Branco",shortPath,String.class);	
	...
	shortPath.clear();
        ...
        Graph<String,String> completeMap2 = new Graph<>(false);
        completeMap2 = completeMap.clone();
        completeMap2.removeEdge("Porto", "Aveiro");
        completeMap2.removeEdge("Porto", "Braga");
        completeMap2.removeEdge("Porto", "Vila Real");
        
        completeMap2.insertEdge("Porto","Aveiro","A1",50);
        completeMap2.insertEdge("Porto","Braga","A3",50);
        completeMap2.insertEdge("Porto","Vila Real","A4",50);
    ...
    */
	...
	assertEquals("Length path should be Double.MAX_VALUE if there is no path", Double.MAX_VALUE, dists.get(completeMap.getKey("Faro")),0.01);
        ...
        assertFalse(GraphAlgorithms.shortestPaths(completeMap,"LX",paths,dists));
